...
const PaymentCallbackPage = () => {
    ...
    useEffect(() => {
        const orderId = searchParams.get('order_id');
        const paymentStatus = searchParams.get('status'); // Get status from URL
        const errorMsg = searchParams.get('message'); // Get error message from URL
        ...
        if (paymentStatus === 'success') {
            ...
            setStatus('error');
            setMessage(decodeURIComponent(errorMsg) || 'Payment failed. Please try again.');
        } else if (paymentStatus === 'pending') {
            setStatus('processing');
            setMessage(decodeURIComponent(errorMsg) || 'Payment is still processing...');
        } else {
            // Fallback for unknown status
            setStatus('processing');
            // Ideally call a GraphQL query here to check status from backend if URL status is missing
        }
    }, [searchParams, navigate, clearCart]);

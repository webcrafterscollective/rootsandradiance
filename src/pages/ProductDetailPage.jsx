...
import { FaStar, FaRegStar, FaCheckCircle, FaInfoCircle, FaTimesCircle, FaShoppingCart, FaMinus, FaPlus } from 'react-icons/fa';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 lg:gap-16 items-start">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="my-5 pt-5 border-t border-brand-subtle space-y-4">
                            {isVariable ? ( 
                                <div className="p-3 border border-blue-500/30 rounded bg-blue-500/10 text-blue-700 dark:text-blue-300 text-sm"> This is a variable product. <Link to={`/products/${product.slug || product.id}`} className="font-medium underline ml-1 hover:text-blue-500 dark:hover:text-blue-200">View Options</Link> </div>
                            ) : !isInStock ? ( 
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center gap-3"> 
                                    ...
                                    <div className="flex items-center"> 
                                        ...
                                        <div className="flex items-center border border-brand-subtle rounded bg-brand-card">
                                            <button
                                                type="button"
                                                onClick={() => setQuantity(q => Math.max(1, q - 1))}
                                                className="px-3 py-2 text-brand-foreground hover:bg-brand-subtle/20 transition-colors"
                                            ...
                                            <input 
                                                ...
                                                value={quantity} 
                                                onChange={(e) => {
                                                    // Allow user to clear input (empty string) or type numbers
                                                    const val = e.target.value;
                                                    if (val === '') setQuantity('');
                                                    else setQuantity(parseInt(val) || 1);
                                                }} 
                                                onBlur={(e) => {
                                                    // Validate bounds on blur
                                                    let val = parseInt(e.target.value) || 1;
                                                    val = Math.max(1, val);
                                                    if (product.stock_quantity) val = Math.min(product.stock_quantity, val);
                                                    setQuantity(val);
                                                }}
                                                className="w-12 border-none bg-transparent text-center text-sm text-brand-foreground focus:ring-0 appearance-none p-0" 
                                                // Note: 'appearance-none' requires Tailwind utilities to hide spinner buttons usually found in number inputs
                                            />
                                            <button
                                                type="button"
                                                onClick={() => setQuantity(q => (product.stock_quantity ? Math.min(product.stock_quantity, (parseInt(q) || 0) + 1) : (parseInt(q) || 0) + 1))}
                                                className="px-3 py-2 text-brand-foreground hover:bg-brand-subtle/20 transition-colors"

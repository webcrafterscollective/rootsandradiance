...
const CheckoutForm = ({ onSubmit, loading }) => {
  ...
  // Set default to 'phonepe' since COD is unavailable
  ...
  const paymentMethods = [
    { 
      id: 'cod', 
      title: 'Cash on delivery (Currently Unavailable)', 
    ...
    { 
      ...
      description: 'Please send a check to our business address.',
      disabled: true 
    },
    { 
      id: 'phonepe', 
      title: 'PhonePe', 
      description: 'Pay with your PhonePe account.',
      disabled: false
    },
  ];
        ...
        <div className="space-y-4 mt-4 rounded-lg border border-gray-200 p-4">
          {paymentMethods.map((method) => (
            <div 
              ...
              } ${method.disabled ? 'opacity-50 cursor-not-allowed bg-gray-50' : ''}`}
            >
              <label className={`flex items-center ${method.disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                <input
                  type="radio"
                  name="paymentMethod"
                  value={method.id}
                ...
                />
                <span className={`ml-3 font-medium ${method.disabled ? 'text-gray-500' : 'text-black'}`}>
                  {method.title}
                </span>
              </label>
              {/* Show description if selected OR if it's the specific unavailable message we want to show */}
              {(paymentMethod === method.id || method.id === 'cod') && (
                <p className={`ml-7 mt-2 text-sm ${method.disabled ? 'text-red-500' : 'text-black'}`}>
                  {method.description}
                </p>
              )}
            </div>
